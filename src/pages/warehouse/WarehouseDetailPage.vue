  ...
  <div class="p-6 flex gap-6">
    ...
    <div class="w-1/6">
      ...
      <div class="bg-white border rounded-xl shadow-sm p-4 space-y-4 mt-5">
        ...
        <div class="border-t pt-3">
          <div class="font-medium text-gray-600 mb-2">선반 랙 리스트</div>

          <div class="text-center" v-if="racks.length == 0">
          ...
          <div class="space-y-2 max-h-96 overflow-y-auto">
            <div
              v-for="rack in racks"
              :key="rack.id"
              class="flex justify-between px-3 py-2 bg-gray-50 rounded-lg"
            >
              <span class="">
    ...
    <div class="w-1/2">
      <div class="bg-white border rounded-xl shadow-sm p-4">
        <h3 class="font-semibold text-gray-700 mb-4">선반 레이아웃 관리</h3>
...
export default {
  ...
  methods: {
    ...
    selectWarehouse(rack) {
      console.log("rack.id", rack.id);
      this.selectedWarehouse = rack;
      this.selectedWarehouse_id = rack.id;
      this.where.warehouse_id = rack.id;
      this.loadData();
    },
    ...
    async save() {
      try {
        ...
        await api.post("/api/location/batchSave", rows);
    ...
    async deleteData(data) {
      try {
        ...
        this.$toast.success(`[${data.name}] 선반정보가 삭제되었습니다`);
        this.loadData();
      } catch (e) {
        this.$toast.error(e.message);
      }
    ...
    async loadData() {
      ...
      const res = await api.post("/api/location/list", where);
      this.racks = res.data;
    },
