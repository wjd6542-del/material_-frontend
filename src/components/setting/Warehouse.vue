...
export default {
  ...
  methods: {
     ...
     * 데이터 로드
     * ========================= */
    // 창고 목록과 컬럼 정의를 서버에서 로드한다
    async loadList() {
      this.rowData = [];
      const res = await api.post("/api/warehouse/list", this.search);
     ...
     * 행 추가
     * ========================= */
    // 최대 sort 값을 계산해 신규 창고 행을 그리드 상단에 추가한다
    addRow() {
      let maxSort = 0;
    ...
    async deleteRows() {
      ...
      try {
        await api.post("/api/warehouse/batchDelete", rows);
