...
export default {
  ...
  methods: {
     ...
     * 데이터 로드
     * ========================= */
    // 카테고리 목록과 컬럼 정의를 서버에서 로드한다
    async loadList() {
      this.rowData = [];
      const res = await api.post("/api/category/list", this.search);
     ...
     * 행 추가
     * ========================= */
    // 최대 sort 값을 계산해 신규 카테고리 행을 그리드 상단에 추가한다
    addRow() {
      let maxSort = 0;
    ...
    async saveRows() {
      ...
      try {
        ...
        await api.post("/api/category/batchSave", rows);
    ...
    // 일괄 삭제 처리
    // 선택된 카테고리를 사용자 확인 후 서버에서 일괄 삭제한다
    async deleteRows() {
      const rows = this.gridApi.getSelectedRows();
      ...
      try {
        await api.post("/api/category/batchDelete", rows);
